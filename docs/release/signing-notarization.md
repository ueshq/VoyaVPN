...
System: Tauri signer, secure key storage, `apps/desktop/src-tauri/tauri.conf.json`, and the stable update host.
...
System: signed Tauri updater payloads, `.sig` files, stable CDN base URL, and `pnpm release -- updater`.
...
```sh
pnpm release -- updater --input dist/release/signed-updater --out dist/updater/latest.json --channel stable --base-url "$VOYAVPN_UPDATES_BASE_URL"
```
...
sing-box Apple/libbox runtime linked into or embedded with the PacketTunnel
extension.

Developer ID Application/Installer certificates are required for the notarized
DMG path. `3rd Party Mac Developer Application` and `3rd Party Mac Developer
