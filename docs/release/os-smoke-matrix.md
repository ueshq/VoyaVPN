...
| Share/export | In Profiles, select one and multiple profiles. Export share links, base64 share links, and Voya node bundles from the toolbar, row context menu, and group header. Confirm collapsed groups export all members and no client-config export is offered. Confirm clipboard contents and the read-only Show QR dialog match the selected profile order. |
