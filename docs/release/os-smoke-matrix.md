...
| Self-hosted node | Platform owner | Every OS package, a second device, and a network with a public address | Enable hosting: the node core starts beside a disconnected app, its config exists only while it runs, and Logs show `[selfhost]` lines without keys. Check network on a public host (the node self-test shows both protocols working), behind a UPnP router, with UPnP off, on CGNAT or a phone hotspot, and IPv6-only; the verdicts and findings match the network. Import the VLESS and SS-2022 links on the second device (VoyaVPN and one third-party client): both connect with default settings and exit from the host's address; `127.0.0.1`, the Clash API port, `169.254.169.254` and a LAN address are refused unless LAN access is on (loopback stays refused). Reset keys: old links stop working. Kill the node core: it restarts; quit: no node core or router forward remains. Windows: Allow adds the firewall rule through UAC. macOS: the local-network prompt names the reason. | Disable hosting, remove router forwards and the firewall rule by hand, and hold the release if the node outlives the app or a private destination is reachable. |
