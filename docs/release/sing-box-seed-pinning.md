...
`VOYAVPN_ALLOW_UNPINNED_SING_BOX=1` covers the two cases where verification is
impossible rather than failed:

- staging a `{version, platform, arch}` with no pinned digest (an upstream
  pre-release, or an architecture the table does not cover yet), and
- accepting an already-staged seed whose `sing-box.seed.json` is missing or
  predates executable-digest pinning, so a developer with no network is not
  blocked by a re-fetch.
...
Without the variable, an unverifiable seed is simply re-staged from upstream and
verified, which is the normal path after a version bump.
