          ...
          <SettingsFields errors={controller.fieldErrors}>
            {tabs.map((item) => <TabsContent key={item.value} className="h-full overflow-y-auto bg-surface-sunken p-4 data-[state=inactive]:hidden" forceMount value={item.value}>
              {visited.has(item.value) ? <div className="mx-auto w-full max-w-5xl">
                {item.value !== "dns" && item.value !== "updates" ? <AppSettingsPane controller={controller} tab={item.value} /> : null}
...
function AppSettingsPane({ controller, tab }: { controller: AppSettingsController; tab: Exclude<SettingsTab, "dns" | "updates"> }) {
  const { t } = useI18n();
  if (!controller.settings) return <p className="text-sm text-muted-foreground" role="status">{controller.working ? t("options.loading") : null}</p>;
  const ready = { ...controller, settings: controller.settings };
  switch (tab) {
    case "general": return <GeneralTab controller={ready} />;
    case "core": return <CoreTab controller={ready} />;
    case "network": return <NetworkTab controller={ready} />;
    case "tests": return <TestsTab controller={ready} />;
  }
}
