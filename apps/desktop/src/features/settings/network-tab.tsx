...
export function NetworkTab({ controller }: { controller: AppSettingsFormController }) {
  const { t } = useI18n();
    ...
    <div className="grid gap-4">
      <SettingsGroup title={t("settings.sections.tun")} actions={<TunDiagnosticsButton />}>
        <div className="grid gap-3 @min-[42rem]:grid-cols-2">
          <SettingsCheckbox field="network.tun.autoRoute" checked={settings.network.tun.autoRoute} label={t("settings.network.tunAutoRoute")} onCheckedChange={(autoRoute) => patchTun({ autoRoute: autoRoute === true })} />
          <SettingsCheckbox field="network.tun.strictRoute" checked={settings.network.tun.strictRoute} label={t("settings.network.tunStrictRoute")} onCheckedChange={(strictRoute) => patchTun({ strictRoute: strictRoute === true })} />
          <SettingsCheckbox field="network.tun.ipv" checked={settings.network.tun.ipv6Enabled} label={t("settings.network.enableIpv6Address")} onCheckedChange={(ipv6Enabled) => patchTun({ ipv6Enabled: ipv6Enabled === true })} />
        </div>
        <SelectField
        ...
        />
        <NumberField field="network.tun.mtu" id="rt-tun-mtu" label={t("settings.network.mtu")} onChange={(mtu) => patchTun({ mtu: mtu ?? 1500 })} value={settings.network.tun.mtu} />
        <SelectField
