...
const mocks = vi.hoisted(() => ({ copyProfiles: vi.fn(), listNodeGroups: vi.fn(), saveNodeGroup: vi.fn(), updateNodeGroup: vi.fn(), exportProfileShareLinks: vi.fn(), exportProfileShareLinksBase64: vi.fn(), exportProfileVoyaBundle: vi.fn(), saveTextFile: vi.fn(), generateQrCode: vi.fn(), deleteNodeGroup: vi.fn(), moveNodeGroup: vi.fn(), assignNodeGroups: vi.fn(), listProfiles: vi.fn(), listSubscriptions: vi.fn(), runSpeedtest: vi.fn(), setActiveProfile: vi.fn(), connectActiveProfile: vi.fn(), restartCore: vi.fn() }));
...
async function menu(name: string, action: string) { if (action === "Edit group") { await userEvent.click(within(card(name)).getByRole("button", { name: action })); return; } await userEvent.click(within(card(name)).getByRole("menuitem", { name: `Actions for ${name}` })); await userEvent.click(await screen.findByRole("menuitem", { name: action })); }
beforeEach(() => {
  vi.resetAllMocks();
  ...
  mocks.updateNodeGroup.mockImplementation(async (id, name, changes: NodeGroupAssignment[]) => {
    const group = groups.groups.find((g) => g.id === id)!;
    group.name = name.trim();
    for (const c of changes) { groups.memberships = groups.memberships.filter((m) => m.profileId !== c.profileId); if (c.groupId) groups.memberships.push({ profileId: c.profileId, groupId: c.groupId }); }
    changed(); return group;
  });
  for (const [exporter, format] of [[mocks.exportProfileShareLinks, "shareLinks"], [mocks.exportProfileShareLinksBase64, "shareLinksBase64"], [mocks.exportProfileVoyaBundle, "voyaBundle"]] as const) exporter.mockImplementation(async (ids: string[]) => ({ text: ids.join("\n"), count: ids.length, format }));
  mocks.saveTextFile.mockResolvedValue("/tmp/nodes.txt");
  mocks.generateQrCode.mockResolvedValue({ mimeType: "image/svg+xml", svg: "<svg></svg>" });
...
afterEach(() => { if (clipboardDescriptor) Object.defineProperty(navigator, "clipboard", clipboardDescriptor); else Reflect.deleteProperty(navigator, "clipboard"); cleanup(); clients.forEach((c) => c.clear()); clients.clear(); });
...
describe("manual node folders", () => {
  ...
  it("has no page search and preserves collapsed groups through query refresh", async () => {
    ...
    expect(screen.getByRole("button", { name: "Backup" })).toHaveAttribute("aria-expanded", "true");
  });
  ...
  it("renames a group without connecting", async () => {
    renderScreen(); await screen.findByRole("button", { name: "Asia" }); await menu("Asia", "Edit group"); const input = screen.getByRole("textbox", { name: "Group name" }); await userEvent.clear(input); await userEvent.type(input, "Travel"); await userEvent.click(screen.getByRole("button", { name: "Save" })); expect(await screen.findByRole("button", { name: "Travel" })).toBeInTheDocument(); expect(mocks.connectActiveProfile).not.toHaveBeenCalled();
  });
  it("deletes a folder while retaining its nodes", async () => {
    renderScreen(); await screen.findByRole("button", { name: "Asia" }); await menu("Asia", "Delete group"); await userEvent.click(within(screen.getByRole("dialog")).getByRole("button", { name: "Delete group" })); await waitFor(() => expect(screen.queryByRole("button", { name: "Asia" })).not.toBeInTheDocument()); expect(screen.getAllByTestId("server-row")).toHaveLength(4);
  });
  it("saves the name and one membership delta together", async () => {
    renderScreen(); await screen.findByRole("button", { name: "Asia" }); await menu("Asia", "Edit group"); const dialog = screen.getByRole("dialog"); await userEvent.click(within(dialog).getByRole("checkbox", { name: "Osaka" })); await userEvent.click(within(dialog).getByRole("checkbox", { name: "Paris" })); fireEvent.change(within(dialog).getByRole("textbox", { name: "Group name" }), { target: { value: "Travel" } }); await userEvent.click(within(dialog).getByRole("button", { name: "Save" }));
    await waitFor(() => expect(mocks.updateNodeGroup).toHaveBeenCalledWith("a", "Travel", [{ profileId: "profile-3", groupId: null }, { profileId: "profile-2", groupId: "a" }])); expect(mocks.setActiveProfile).not.toHaveBeenCalled();
  });
  it("keeps member edits and reports a failed save", async () => {
    mocks.updateNodeGroup.mockRejectedValue(new Error("Membership save failed")); renderScreen(); await screen.findByRole("button", { name: "Asia" }); await menu("Asia", "Edit group"); await userEvent.click(screen.getByRole("checkbox", { name: "Paris" })); await userEvent.click(screen.getByRole("button", { name: "Save" })); expect(await screen.findByRole("alert")).toHaveTextContent("Membership save failed"); expect(screen.getByRole("checkbox", { name: "Paris" })).toBeChecked();
  });
  it("tests all members even when the group is collapsed", async () => {
    renderScreen(); await screen.findByRole("button", { name: "Asia" }); await userEvent.click(screen.getByRole("button", { name: "Asia" })); await userEvent.click(within(card("Asia")).getByRole("button", { name: "Test group" })); expect(mocks.runSpeedtest).toHaveBeenCalledWith({ target: { scope: "profiles", profileIds: ["profile-0", "profile-3"] } });
  });
  ...
  it.each(["disconnected", "connected"] as const)("uses the ordinary activation flow while %s", async (state) => {
    ...
    await userEvent.click(within(screen.getByText("Paris").closest("article")!).getByRole("button", { name: "Use node" })); expect(mocks.setActiveProfile).toHaveBeenCalledWith("profile-2"); await waitFor(() => expect(state === "connected" ? mocks.restartCore : mocks.connectActiveProfile).toHaveBeenCalledOnce());
  });
  it("flattens large lists with stable node identities", () => {
    ...
    const rows = nodeListRows(many, snapshot, new Set(), "Unassigned"); expect(rows).toHaveLength(5002); expect(new Set(rows.map((r) => r.key)).size).toBe(rows.length);
  });
  ...
  it("selects search results in the member dialog and cancels without writing", async () => {
    ...
    await waitFor(() => expect(within(card("Asia")).getByRole("button", { name: "Edit group" })).toHaveFocus());
  });
  it("keeps a failed rename editable and blocks duplicate submissions", async () => {
    let reject!: (error: Error) => void;
    mocks.updateNodeGroup.mockImplementation(() => new Promise((_, rejectSave) => { reject = rejectSave; }));
    renderScreen(); await screen.findByRole("button", { name: "Asia" });
    await menu("Asia", "Edit group");
...
describe("group panels and scoped export", () => {
  async function exportGroup(name: string, action: string) {
    await userEvent.click(screen.getByRole("menuitem", { name: `Export ${name}` }));
  ...
  it("marks continuous panel boundaries, including empty and unassigned groups", () => {
    ...
    const rows = nodeListRows(profiles, { ...groups, groups: [...groups.groups, empty] }, new Set(), "Unassigned");
    expect(rows.map((row) => [row.key, row.groupKey, row.last])).toEqual([
      ["group:a", "a", false], ["profile:profile-0", "a", false], ["profile:profile-3", "a", true],
      ["group:b", "b", false], ["profile:profile-1", "b", true],
      ["group:empty", "empty", true], ["group:unassigned", UNASSIGNED_GROUP_KEY, false], ["profile:profile-2", UNASSIGNED_GROUP_KEY, true],
    ]);
    const collapsed = nodeListRows(profiles, groups, new Set(["a", "b", UNASSIGNED_GROUP_KEY]), "Unassigned");
  ...
  it("shows an empty panel and disables its export", async () => {
    ...
    renderScreen(); await screen.findByRole("button", { name: "Backup" });
    expect(within(card("Backup")).getByText(/No nodes in this group/)).toBeInTheDocument();
    expect(screen.getByRole("menuitem", { name: "Export Backup" })).toBeDisabled();
    expect(within(card("Unassigned")).queryByRole("button", { name: "Edit group" })).not.toBeInTheDocument();
  });
  ...
  ] as const)("exports fresh complete membership as %s while collapsed", async (label, command) => {
    renderScreen(); await screen.findByRole("button", { name: "Asia" });
    await userEvent.click(screen.getByRole("button", { name: "Asia" }));
    groups.memberships.push({ profileId: "profile-2", groupId: "a" });
    await exportGroup("Asia", label);
    await waitFor(() => expect(mocks[command]).toHaveBeenCalledWith(["profile-0", "profile-2", "profile-3"]));
  ...
  it("exports only unassigned nodes and keeps removed export formats absent", async () => {
    renderScreen(); await screen.findByRole("button", { name: "Asia" });
    await userEvent.click(screen.getByRole("menuitem", { name: "Export Unassigned" }));
    expect(screen.getAllByRole("menuitem").map((item) => item.textContent).filter((text) => text?.includes("Client config") || text?.includes("Save client config"))).toEqual([]);
  ...
  it("saves group share links through the existing file dialog", async () => {
    ...
    await waitFor(() => expect(mocks.saveTextFile).toHaveBeenCalledWith({ defaultPath: "voyavpn-share-links.txt", filters: [{ extensions: ["txt"], name: "Text" }], text: "profile-0\nprofile-3" }));
  ...
  it.each(["deleted", "load failed", "empty"])("does not fall back to exporting all nodes when a group is %s", async (failure) => {
    renderScreen(); await screen.findByRole("button", { name: "Asia" });
    if (failure === "deleted") groups.groups = groups.groups.filter((group) => group.id !== "a");
    else if (failure === "empty") groups.memberships = groups.memberships.filter((m) => m.groupId !== "a");
    else mocks.listNodeGroups.mockRejectedValue(new Error("Groups unavailable"));
  ...
  it("skips unsupported share protocols but includes them in a Voya bundle", async () => {
    const http = makeProfileFixture(4, { protocol: { kind: "http", server: { address: "http.test", port: 8080 }, username: "", password: "" } });
    mocks.listProfiles.mockResolvedValue({ entries: [...profiles, http], undecodableProfiles: 0 });
    ...
    await waitFor(() => expect(mocks.exportProfileVoyaBundle).toHaveBeenCalledWith(["profile-0", "profile-3", "profile-4"]));
  });
