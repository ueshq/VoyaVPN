...
export function NodeGroupDialogs({ controller }: { controller: ServerTableController }) {
...
function GroupError({ controller }: { controller: ServerTableController }) {
  return controller.nodeGroups.error ? <p className="text-sm text-destructive" role="alert">{controller.nodeGroups.error}</p> : null;
}

function GroupNameForm({ controller, group }: { controller: ServerTableController; group: NodeGroup | null }) {
...
function GroupEditForm({ controller, group }: { controller: ServerTableController; group: NodeGroup }) {
  ...
  const members = useMemo(() => profiles.filter(({ profile }) => `${profile.remarks}\n${profileAddress(profile)}`.toLocaleLowerCase().includes(search.trim().toLocaleLowerCase())), [profiles, search]);
  const groupNames = new Map(nodeGroups.snapshot.groups.map((g) => [g.id, g.name]));
  const assignments = new Map(nodeGroups.snapshot.memberships.map((m) => [m.profileId, m.groupId]));
  const virtualizer = useVirtualizer({ count: members.length, getScrollElement: () => viewport.current, estimateSize: () => 64, getItemKey: (index) => members[index]!.profile.id, overscan: 5, initialRect: { width: 450, height: 320 } });
  const visible = virtualizer.getVirtualItems();
  const rendered = visible.length ? visible : members.slice(0, 10).map((member, index) => ({ index, start: index * 64, key: member.profile.id }));
    ...
    <div className="h-[clamp(8rem,calc(100dvh-26rem),20rem)] overflow-auto rounded border" ref={viewport} tabIndex={0} aria-label={t("nodeGroups.members")} onKeyDown={(event) => navigateVirtualList(event, members.length, (index) => virtualizer.scrollToIndex(index), "data-member-index", '[role="checkbox"]')}>
      <div className="relative" style={{ height: virtualizer.getTotalSize() }}>
        {rendered.map((row) => {
          ...
          const source = groupNames.get(assignments.get(member.id) ?? "") ?? t("nodeGroups.unassigned");
          return <label className="absolute start-0 top-0 flex h-16 w-full cursor-pointer items-center gap-3 px-3 hover:bg-muted" key={member.id} data-member-index={row.index} style={{ transform: `translateY(${row.start}px)` }}>
            <Checkbox checked={selected.has(member.id)} disabled={nodeGroups.busy} onCheckedChange={() => toggle(member.id)} aria-label={member.remarks || member.id} />
            <span className="min-w-0"><span className="block truncate" title={member.remarks}>{member.remarks || member.id}</span><span className="block truncate text-xs text-muted-foreground" title={`${profileAddress(member)} · ${source}`}>{profileAddress(member)} · {source}</span></span>
          </label>;
        })}
      </div>
