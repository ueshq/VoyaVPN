          ...
          <div aria-label={t("status.loadingScreen")} aria-busy="true" className="grid gap-3" role="status">
            {Array.from({ length: 6 }, (_, index) => (
              <div className="node-card-surface" key={index}>
                ...
                <div className="grid flex-1 gap-2"><Skeleton className="h-3 w-24" /><Skeleton className="h-5 w-3/4" /><Skeleton className="h-3 w-1/2" /></div>
                        ...
                        <button aria-label={t("panes.profiles.card.select", { name: rawName })} aria-pressed={selected} data-row-focus className="node-card-select" onClick={() => selectOnly(id)} type="button">
                          <span className="node-card-label">
                            ...
                            {running ? <span className="node-card-state"><span aria-hidden="true" className="size-1.5 rounded-full bg-connected" />{t("panes.profiles.aria.activeProfile")}</span>
                              : item.isActive ? <span className="shrink-0">{t("panes.profiles.card.default")}</span> : null}
                          </span>
                          <span className="node-card-name" title={rawName}>{name}</span>
                        </button>
                        <div className="node-card-meta">
                          <span className="node-card-address" title={address}>{address}</span>
                          <span>{getProtocolLabel(profile.protocol.kind)}</span>
                          <button className="node-card-details" onClick={(event) => openDetails(id, event.currentTarget)} type="button">
                            {t("panes.profiles.card.details")}<ChevronRight aria-hidden="true" className="size-3" />
                          </button>
                        </div>
                      </div>
                      <div className="profile-node-actions">
                        <span className="profile-node-latency" title={t("panes.profiles.cardFields.delay")}>{profileLatency(item, t)}</span>
                        <button aria-busy={switching || undefined} className="node-card-action" disabled={activation.busy || running} onClick={() => void activation.activateProfile(id)} type="button">
                          {switching ? <LoaderCircle aria-hidden="true" className="size-4 animate-spin" /> : null}
                          {switching ? t("panes.profiles.card.switching") : running ? t("panes.profiles.card.using") : t("panes.profiles.card.use")}
                        </button>
