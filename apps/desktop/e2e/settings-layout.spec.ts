...
for (const locale of ["en", "zh-Hans"] as const) {
  for (const theme of ["light", "dark"] as const) {
    test(`manual proxy settings fit the minimum window in ${locale} ${theme}`, async ({
      page,
    }, testInfo) => {
      ...
      await expect(
        page.getByText(
          locale === "en" ? "Manual proxy setup" : "手动代理配置",
          { exact: true },
        ),
      ).toHaveCount(0);
      await expect(
        page.getByText(
          locale === "en" ? "Applies on the next connection" : "下次连接生效",
        ),
      ).toHaveCount(0);
      await page
        .getByRole("tab", {
      ...
      await page.screenshot({
        path: testInfo.outputPath("manual-proxy-settings.png"),
      });
      ...
      await expect(
        panel.getByRole("button", {
          name: locale === "en" ? "Copy address" : "复制地址",
        }),
      ).toHaveCount(0);
    });
  }
