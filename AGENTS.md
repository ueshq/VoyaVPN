...
Two environment variables are read outside the scripts. `VOYAVPN_LOG` sets the
app's tracing filter (same syntax as `RUST_LOG`, which it falls back to).
...
- **voya-mobile-ffi** — The mobile host: the second shell, in the same sense `apps/desktop/src-tauri` is the first. It owns a tokio runtime, calls `AppServices::connect`, injects a rejecting `ProcessRunner` (a phone spawns nothing — the tunnel provider runs the core) and a `NativeTunController` that delegates to the host app, and turns the `voya-app` sinks it wires (supervisor, proxy runtime) plus its own command outcomes into the three event channels; a phone runs no statistics sampler, auto-update scheduler or child process, so it implements none of those sinks. Its uniffi surface is an envelope, not a model: `invoke(command, args_json) -> json` in the exact wire shape Tauri uses, plus `EventListener` / `TunnelHost` / `ProbeCoreHost` callbacks — see ADR 0012. Two generated files stand in for the types uniffi does not check: `packages/contracts/commands.json` (every command must have a dispatcher or be on `UNSUPPORTED_ON_MOBILE`, never both) and `packages/contracts/events.json` (the channel names it must emit under; the payloads are `voya-contracts` types both hosts share).
