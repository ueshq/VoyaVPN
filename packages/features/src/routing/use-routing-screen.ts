...
export function useRoutingScreen() {
  ...
  async function saveRule(rule: RoutingRulePayload) {
    ...
    const saved = await runOperation(
      (routingId) => voyaCommands().saveRoutingRule(routingId, rule),
  ...
  async function toggleRule(rule: RoutingRule, enabled: boolean) {
    ...
    try {
      await runOperation((routingId) => voyaCommands().saveRoutingRule(routingId, { ...rule, enabled }));
    } finally {
  ...
  function moveRule(rule: RoutingRule, action: RuleMoveAction) {
    void runOperation((routingId) =>
      ...
      action === "top" && offset > 0
        ? voyaCommands().moveRoutingRule(routingId, rule.id, "position", offset)
        : voyaCommands().moveRoutingRule(routingId, rule.id, action, null),
    );
  }
  ...
  function reorderRule(ruleId: string, from: number, to: number) {
    ...
    return runOperation(
      (routingId) => voyaCommands().moveRoutingRule(routingId, ruleId, "position", position),
  ...
  function confirmPending() {
    ...
    void runOperation((routingId) => {
      switch (pending.kind) {
        case "resetRules":
          return voyaCommands().resetRoutingRules(routingId);
        case "deleteRule":
          return voyaCommands().deleteRoutingRules(routingId, [pending.rule.id]);
        case "fixOutbound":
          // The node or group the rule named is gone; the proxy always exists.
          return voyaCommands().saveRoutingRule(routingId, { ...pending.rule, outbound: "proxy" });
      }
    });
  }
