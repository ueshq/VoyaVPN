...
This is the Claude Code variant of spec-plan-rollout/scripts/generate_rollout.py.
The generated runner shells out to the Claude Code CLI (`claude -p`) instead of
`codex exec`. Plan YAML uses `claude_cmd` and `execution: claude`.
"""
...
#                      [--force] [--dry-run] [--commit-per-batch | --no-commit-per-batch] [--claude-cmd CMD] [--model MODEL]
...
DEFAULT_CLAUDE_CMD = "claude -p --dangerously-skip-permissions --output-format text"
        ...
        for raw_batch in raw_phase["batches"]:
            ...
            batches.append(
                Batch(
                    ...
                    execution=raw_batch.get("execution") or "claude",
                    goal=raw_batch["goal"],
...
def write_prompt(phase: Phase, batch: Batch, attempt: int, extra_notes: str | None) -> Path:
    ...
    path.write_text(render_prompt(phase, batch, extra_notes=extra_notes), encoding="utf-8")
    return path


# On Windows, route shell-string commands through bash when available so that
# verify_commands written in POSIX style (grep, test, &&-chains, single-quoted
# strings) work the same as under bash on macOS/Linux. Without this, shell=True
# on Windows defaults to cmd.exe, which has no `grep` and parses quoting
# differently. Falls back to cmd.exe only if bash isn't on PATH.
def _detect_windows_bash() -> str | None:
    if os.name != "nt":
        return None

    # Windows ships C:\Windows\System32\bash.exe (and the WindowsApps shim) as
    # the WSL launcher. If WSL isn't installed it errors with
    # REGDB_E_CLASSNOTREG. Prefer Git Bash and explicitly skip WSL stubs.
    ...
    if git_on_path:
        git_root = Path(git_on_path).resolve().parent
        # git.exe usually lives in <root>/cmd or <root>/bin; bash.exe in <root>/bin.
        for ancestor in {git_root, git_root.parent}:
    ...
    if fallback and not _is_wsl_stub(fallback):
        return fallback

    return None


_BASH_EXECUTABLE = _detect_windows_bash()
...
def invoke_claude(
    ...
    *,
    ...
    if dry_run:
        print(c("  (dry-run, skipping claude invocation)", Colors.YELLOW))
    ...
    with prompt_path.open("rb") as stdin, log_path.open(mode) as log:
        if attempt > 0:
            log.write(b"\n")
        log.write(f"# claude invocation {attempt + 1} for {batch.id}\n".encode())
        log.write(f"# cmd: {shlex.join(claude_cmd)}\n".encode())
        ...
        # `claude -p --output-format text` buffers its entire response until
        # generation finishes, and on Windows the stdout pipe is inherited by
        # every helper process the CLI spawns. A synchronous `for line in
        # proc.stdout` makes long batches look hung and can deadlock if a
        # helper outlives claude.exe. Pump in a daemon thread, heartbeat while
        # waiting on proc.wait(), and drain with a grace period on exit.
        ...
        while True:
            try:
                rc = proc.wait(timeout=heartbeat_interval)
                break
                ...
                if idle >= heartbeat_interval:
                    ...
                    print(c(
                        f"  ... claude still running ({elapsed:.0f}s elapsed, "
                        f"{idle:.0f}s since last output)",
                        Colors.DIM,
                    ))

        # proc.wait() returned the authoritative exit code. If helper
        # subprocesses are still holding the pipe (Windows handle inheritance),
        # don't wait for them — drain whatever's buffered, then move on.
        ...
        with output_lock:
            captured = bytes(output_buf).decode("utf-8", errors="replace").rstrip()
        return rc, prompt_path, captured
...
def build_claude_retry_notes(batch: Batch, claude_failure: ClaudeFailure, retry_number: int) -> str:
    return "\n".join(
        [
            f"The previous Claude Code CLI attempt for batch `{batch.id}` exited with a non-zero status.",
            ...
            f"Exit code: `{claude_failure.exit_code}`",
            "Output:",
            "```text",
            claude_failure.output,
            "```",
            "",
        ]
    )
...
def resolve_executable_command(command: str) -> list[str]:
    ...
    if executable is None:
        print(c(f"! 未找到命令 `{command}`。请安装 Claude Code CLI，或使用 --claude-cmd 覆盖。", Colors.RED))
        sys.exit(2)
...
def resolve_claude_cmd(user_cmd: str | None, model: str | None) -> list[str]:
    template = user_cmd or ROLLOUT.get("claude_cmd") or DEFAULT_CLAUDE_CMD
    rendered = template.format(repo=str(REPO))
    cmd = split_command_line(rendered)
    require(bool(cmd), "Claude Code command is empty.")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--dry-run", action="store_true", help="Write prompts only, do not invoke Claude Code")
    ...
    parser.add_argument("--claude-cmd", help="Override the Claude Code command template")
    parser.add_argument("--model", help="Override the Claude Code model")
    parser.add_argument("--reset-batch", metavar="BATCH_ID", help="Reset one batch to pending state")
    parser.add_argument(
        "--max-fix-attempts",
        type=int,
        default=None,
        help="Retries after Claude Code or verification failures; defaults to the plan value",
    )
...
def main() -> int:
    ...
    if args.dry_run:
        ...
        claude_cmd = resolve_claude_cmd(args.claude_cmd, model)
        print(c(f"claude cmd: {shlex.join(claude_cmd)}", Colors.DIM))
    ...
    for batch_id in selected_batch_ids:
        ...
        while True:
            rc, prompt_path, claude_output = invoke_claude(
                phase,
                batch,
                claude_cmd,
                log_path,
            ...
            if rc != 0:
                claude_failure = ClaudeFailure(
                    exit_code=rc,
                    output=truncate_output(claude_output or "(no output)"),
                )
                if attempt < max_fix_attempts:
                    attempt += 1
                    extra_notes = build_claude_retry_notes(batch, claude_failure, attempt)
                    print(c(f"↺ {batch.id} claude exited with {rc}, retrying ({attempt})", Colors.YELLOW))
                    continue
                if not args.dry_run:
                    mark_batch(
                        state,
                        ...
                        claude_failure={
                            "exit_code": claude_failure.exit_code,
                            "output": claude_failure.output,
                        },
                    )
                print(c(f"✗ {batch.id} claude exited with {rc} ({elapsed:.0f}s)", Colors.RED))
                return rc
...
def validate_verify_commands(commands: list[str], field_name: str) -> list[str]:
    for index, command in enumerate(commands):
        normalized = " ".join(command.split())
        require(
            "pnpm test -- --run" not in normalized,
            f"{field_name}[{index}] uses `pnpm test -- --run`, which can pass "
            "a literal `--run` to Vitest and leave it in watch mode. Use "
            "`pnpm test --run` or `pnpm exec vitest --run`.",
        )
        require(
            "vitest -- --run" not in normalized,
            f"{field_name}[{index}] uses `vitest -- --run`, which can leave "
            "Vitest in watch mode. Use `vitest --run`.",
        )
    return commands
...
def validate_rollout(raw: dict, plan_path: Path) -> dict:
    ...
    if rollout["claude_cmd"] is not None:
        rollout["claude_cmd"] = as_string(rollout["claude_cmd"], "rollout.claude_cmd")
...
def validate_phases(raw_phases: object) -> list[dict]:
    ...
    for phase_index, raw_phase in enumerate(raw_phases):
        ...
        for batch_index, raw_batch in enumerate(raw_batches):
            ...
            verify_commands = validate_verify_commands(
                as_string_list(raw_batch.get("verify_commands", []), f"{batch_id}.verify_commands"),
                f"{batch_id}.verify_commands",
            )
            ...
            batches.append(
                {
                    ...
                    "kind": batch_kind,
                    "execution": as_choice(raw_batch.get("execution", "claude"), f"{batch_id}.execution", {"claude"}),
...
def main() -> int:
    ...
    output_path.write_text(render_runner(plan), encoding="utf-8")
