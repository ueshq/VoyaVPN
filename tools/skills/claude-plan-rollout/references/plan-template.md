...
- Prefer test commands that explicitly run once, such as `pnpm test --run` for Vitest under pnpm.
...
- Do not write `pnpm test -- --run` for Vitest; pnpm passes the extra separator through and Vitest can enter watch mode. Use `pnpm test --run`.
