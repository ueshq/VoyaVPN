...
This spec is the upstream decision source for the implementation plan. Draft or
update it before writing `plan.md`; the plan should derive phase goals, batch
boundaries, acceptance, risks, and verification from this document.
...
- Codex CLI assumptions: `codex exec` is on PATH, `--dangerously-bypass-approvals-and-sandbox` is acceptable for the target repo, model id is set when overriding the default.
