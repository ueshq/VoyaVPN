...
description: >-
  ...
  the generated runner drives Codex through the terminal: a human-readable
  spec, a phase-and-batch execution plan, and an optional generated
  `rollout.py` runner. Use when Codex needs to orchestrate refactors,
...
3. Draft a project-specific implementation plan at `.agents/rollouts/<runner-name>/plan.md`.
   ...
   unit one Codex invocation should finish safely.
...
- Keep `execution` set to `codex`; the generated runner is fully automatic and
...
When a batch must make a genuine runtime decision, give Codex the authority and
the default in the prompt context. For example, if two files would collide,
tell it to delete one when they are functionally identical or rename to a
documented basename and record the choice in evidence that downstream batches
can read.
...
- calls `codex exec ... -` for each batch, piping the rendered prompt to Codex
  on stdin
- verifies each batch with shell commands
- feeds failed Codex or verification output back into a retry prompt
...
- Default command template:
  `codex exec --dangerously-bypass-approvals-and-sandbox --cd {repo} -`. Override
  per project via `rollout.codex_cmd` in the plan YAML or `--codex-cmd` at
  runtime; use `{repo}` for the absolute repo root.
- Set a model with `rollout.model` or `--model`; the runner inserts
  `--model <id>` before the stdin marker (`-`) when set.
- `--dangerously-bypass-approvals-and-sandbox` is required for unattended
  execution because Codex otherwise asks for approvals or runs in a restricted
  sandbox. Only run the generated `rollout.py` in repositories where that is
  acceptable.
- The working tree must be clean unless `allow_dirty` (plan) or
  `--allow-dirty` (CLI) is set.
- A failed Codex invocation or failed verification is fed back as a retry
...
Each `rollout.py` is single-threaded: one batch invokes one Codex process and
waits for it to exit. The runner does not coordinate with other rollouts
...
`rollout.codex_cmd` at a wrapper that queues invocations.
